...
# Chinese text normalization (TN/ITN) for ASR evaluation.
#
# Copyright (c) 2026 SpeechColab. Licensed under the MIT License (see LICENSE.gigaspeechbench).
#
# Portions of this file are derived from:
#   - speechio/chinese_text_normalization (MIT License)
#     https://github.com/speechio/chinese_text_normalization
#     the NSW normalization machinery: ChineseChar / NumberSystem / chn2num /
#     num2chn / Cardinal / Digit / TelePhone / Fraction / Date / Money /
#     Percentage / normalize_nsw / TextNorm.
#     That project credits atomicoo/chn_text_norm for the NSW normalizers.
#   - Zhon (MIT License), https://github.com/tsroten/zhon
#     the Chinese punctuation constants.
...
# ──────────────────────────────────────────────
# Hallucination detection & cleaning
# ──────────────────────────────────────────────

def _tokenize_for_hallucination(text: str) -> list:
    tokens = []
    buf = []
    for ch in text:
...
def _detokenize(tokens: list) -> str:
    ...
    for i, tok in enumerate(tokens):
        if i == 0:
            ...
            continue
        prev = tokens[i - 1]
        prev_is_cjk = len(prev) == 1 and unicodedata.category(prev) == "Lo"
        cur_is_cjk = len(tok) == 1 and unicodedata.category(tok) == "Lo"
        if prev_is_cjk and cur_is_cjk:
            parts.append(tok)
        elif prev_is_cjk or cur_is_cjk:
            parts.append(tok)
        else:
            parts.append(" " + tok)
    return "".join(parts)
...
def _clean_hallucination(text: str, max_ngram_length: int = 30,
                         max_occurrences: int = 8) -> str:
    ...
    changed = False
    while True:
        max_repeat = {"n": 0, "start": 0, "end": 0, "count": 0}
        for n in range(1, min(max_ngram_length, len(cleaned_tokens)) + 1):
            i = 0
            while i <= len(cleaned_tokens) - n:
                current_ngram = tuple(cleaned_tokens[i:i + n])
                repeat_count = 1
                j = i + n
                while j <= len(cleaned_tokens) - n:
                    if tuple(cleaned_tokens[j:j + n]) == current_ngram:
                        repeat_count += 1
                        j += n
                    else:
                        break
                if (repeat_count > max_repeat["count"] or
                        (repeat_count == max_repeat["count"] and n > max_repeat["n"])):
                    max_repeat = {"n": n, "start": i, "end": i + n, "count": repeat_count}
                i = j if repeat_count > 1 else i + 1
        if max_repeat["count"] >= max_occurrences:
            changed = True
            keep_end = max_repeat["start"] + max_repeat["n"] * 5
            total_end = max_repeat["start"] + max_repeat["n"] * max_repeat["count"]
            cleaned_tokens = cleaned_tokens[:keep_end] + cleaned_tokens[total_end:]
        else:
            break
    return _detokenize(cleaned_tokens) if changed else text
...
# https://zh.wikipedia.org/wiki/Halfwidth_and_Fullwidth_Forms
QJ2BJ = {
    "　": " ",
    "！": "!",
    "＂": '"',
...
# 2013 China National Standard (8105 common Chinese characters):
...
class ChineseChar(object):
    """
    Chinese character
    Each character has a simplified and a traditional form,
...
class ChineseNumberUnit(ChineseChar):
    """
    Chinese number/unit character
    In addition to simplified/traditional, each character has an extra capital form
    e.g. '陆' and '陸'
    """
...
class MathSymbol(object):
    """
    Math symbols for the Chinese number system (traditional/simplified), e.g.
    positive = ['正', '正']
    negative = ['负', '負']
    point = ['点', '點']
    """
...
def create_system(numbering_type=NUMBERING_TYPES[1]):
    """
    Create and return the corresponding number system based on the numbering type; defaults to mid
...
def normalize_nsw(raw_text):
    ...
    # Normalize pure numbers + measure words
...
def remove_erhua(text):
    """
    Remove the 儿 in erhua (rhotic) words:
    他女儿在那边儿 -> 他女儿在那边
    """
...
# ================================================================================ #
#         format5res (cn_itn): ITN (Chinese numerals -> Arabic numerals) and character-level tokenization
#         The functions below are merged from the original format5res.py, matching the original behavior verbatim
# ================================================================================ #
...
# ================================================================================ #
#   Generic text-normalization entry point (faithfully reproduces whisper_mix_normalize.py behavior)
#   A single function handles both Chinese and English; it detects language per token,
#   English tokens take the en branch; Chinese/mixed tokens take the chn_en / not_chn_en branch.
#   Both CHN.py and USA.py reuse this module's normalize().
# ================================================================================ #
...
def _normalize_part(part, language):
    """Process a text segment by language branch (reproduces the three branches of whisper_mix_normalize.py)"""
    s = " ".join(part)
    if language == "en":
        out_part1 = english_normalizer(s)
        return scoreformat("", out_part1)
    elif language == "chn_en":
        out_part1 = english_normalizer(s)
        out_part2 = normalize_nsw(out_part1)
        out_part3 = all_convert(out_part2)
        return zhconv.convert(out_part3, "zh-cn")
    else:
        out_part1 = basic_normalizer(s)
        out_part2 = normalize_nsw(out_part1)
        out_part3 = all_convert(out_part2)
        return zhconv.convert(out_part3, "zh-cn")
...
def normalize(text: str) -> str:
    """
    Generic text normalization (unified Chinese/English), faithfully reproducing whisper_mix_normalize.py behavior:

    1. Preprocessing: replace ( ) = with spaces, remove /sil (keep text inside brackets)
    2. Tokenize on whitespace, detect language per token (en / chn_en / not_chn_en)
    3. Merge consecutive same-language tokens into a segment, process per language branch:
       - en        : english_normalizer -> scoreformat (English normalization + tokenization, no Chinese TN/ITN)
       - chn_en    : english_normalizer -> normalize_nsw -> all_convert -> zhconv
       - not_chn_en: basic_normalizer  -> normalize_nsw -> all_convert -> zhconv

    The same function works for both English and Chinese: language detection happens at token granularity.
    normalize_nsw does TN (numbers -> Chinese), all_convert does ITN (Chinese -> Arabic numerals) and tokenizes by character.

    Example:
        normalize("我有123个苹果")              -> "我 有 1 2 3 个 苹 果"
        normalize("I have 123 apples")        -> "i have 1 2 3 apples"
    """
    # Step 0: clean hallucination (repeated n-grams) before normalization
    text = _clean_hallucination(text)

    text = text.strip()

    # Preprocessing (same as the original)
    text = re.sub(r"\(", " ", text)
    text = re.sub(r"\)", " ", text)
    text = text.replace("/sil", "")
    text = text.replace("<|nospeech|>", "")
    # Note: do not replace '=', preserve it
    ...
    for i in range(n):
        tok = arr[i]
        chn_eng_bool = is_only_chinese_and_english(tok)
        eng_bool = is_only_english(tok)
        num_bool = is_number(tok)
        ...
        if language == language_bak or language_bak == "":
            ...
            conts.append(_normalize_part(part, language_bak))
            language_bak = language
            part = [tok]

        if i == n - 1:
            conts.append(_normalize_part(part, language))

    # Collapse extra spaces into a single space (matches coworker's normalization; no effect on WER/CER)
    return re.sub(r"\s+", " ", " ".join(conts)).strip()


if __name__ == "__main__":
    print(normalize("自己会去处理它， (noise) 然后一步一回脚印儿继续走下它。"))
    print(normalize("I have 123 apples and 50% off"))
